...
function getAdopApiBase() {
  ...
  // Priority 3: built-in hosted backend (works out of the box for the packaged app,
  // no manual setup needed). Only used when loaded as a local file (the APK); when
  // the site itself is opened as a normal website, same-origin /api/* is used instead.
  if (isFileProtocol()) return 'https://theadop-server.vercel.app';
  // Priority 4: running as a normal website — call the same origin's /api/* routes.
  return '';
}
